...
- **Palette**: Matcha (see [docs/brand/matcha.html](docs/brand/matcha.html)).
  ...
  - Secondary accent: ochre gold `#A88840` — highlights, favorites, special-item flag, AND (systemic rule) **"gold marks the data"**: numbers, dates, and values across the product render in gold. Labels and UI chrome stay neutral. Magazine-folio convention. One recitable rule.
    ...
    - **The unit travels with the number.** A data expression is the *whole* expression including its unit: `8 pieces` is one gold phrase, `24×` is one gold phrase, `$3,815` is one gold phrase, `In 3 days` is one gold phrase. Don't gold just the digit and leave the unit in neutral — that fragments the data into a two-color phrase and reads as a clash, not as emphasis. The exception is **separate label + badge** patterns (e.g., a `Filters` button with a small `2` badge appended): there the label is chrome, the count is data, and the two-color split is correct because they're conceptually distinct elements.
  ...
  - **State colors** (tinted toward brand, never stock web-default):
    - Error: muted terracotta `#A85040` light / `#D08070` dark. Warm, earthy, echoes matcha's register.
    - Success: reuse `--accent` (matcha green itself). A confirmed action is confirmed IN the brand color.
    - Warning: reuse `--gold-dk` (ochre gold deepened). Extends the gold-as-attention rule.
  - **Color system posture** (for the future token migration to `apps/web/app/globals.css`):
    - OKLCH is canonical in the app's globals.css (shadcn ecosystem default). Hex values here and in matcha.html are the human-readable reference. Both agree on actual colors; only representation differs.
    - Ramp stays at 3 steps per family (dk / base / lo). Hover / focus / disabled states handled via opacity modifiers (`bg-primary/90`, `bg-primary/50`) or `color-mix()`, not additional ramp tokens. Keeps token count small and rules memorable.
  - **Dark mode**: own register, NOT a light-mode inversion. Brighter matcha green (`#9EBF94`), warmer gold (`#DCBB6A`), lit-from-within card surfaces. See `.dark-band` in matcha.html for per-value contrast ratios.
...
- **Shape**: hard edges via a 2-token radius scale. `--radius-structural: 0px` on cards, dividers, semantic markers, and surfaces (retains architectural hard-edge identity). `--radius-interactive: 2px` on buttons, inputs, chips, badges, and tags (reads touchable without becoming soft). 1.5px borders for precision. Cards are rectangles with a border, not floating rounded panels. Never large pillows (≥8px). Principle 4: precision should never fight use.
