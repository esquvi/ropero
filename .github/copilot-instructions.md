...
- **Palette**: Matcha. Primary accent matcha green `#5A7852`, secondary ochre gold `#A88840`, warm off-white bg `#EEEFE8`, near-black dark `#0C0F0A`. Dark mode is an own-register (brighter matcha `#9EBF94`, warmer gold `#DCBB6A`), not an inversion. State colors are tinted toward brand: error `#A85040` (muted terracotta), success = accent, warning = gold-dk. OKLCH canonical in globals.css, hex in brand docs. 3 ramp steps per family; use opacity modifiers for hover/focus/disabled.
- **Gold rule (systemic, not per-screen)**: gold marks the data. Numbers, dates, prices, wear counts, durations, scores, IDs all render in gold (`text-gold`) with `tabular-nums`. Labels, headings, item names, brand names, body copy, button text, and chrome stay neutral. Apply on every surface that renders data — a surface with zero gold means a surface with zero data, which is vanishingly rare in this app. **The unit travels with the number**: `8 pieces`, `24×`, `$3,815`, `In 3 days` — the whole expression goes gold including its unit suffix or currency prefix, never just the digit while the unit stays neutral (that's a two-color clash, not emphasis). Exception: label + badge patterns (e.g., `Filters 2` button) keep the label neutral and the badge gold because they're conceptually separate elements.
